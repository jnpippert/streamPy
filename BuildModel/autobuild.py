# streamPy imports
from .constants import sectors, slope_dict, direction_dict
...
class Model:
    ...
    def __init__(self, original_data : np.ndarray,
        ...
        if any([val == 0 for val in[init_x,init_y]]):
            raise ValueError(f"invalid box center positions (expected values > 0, got {init_x,init_y})")
        
        if any([val == 0 for val in[init_width, init_height]]):
            raise ValueError(f"invalid inital box dimensions (expected values > 0, got {init_width}x{init_height})")
    ...
    def _chek_termination(self):
        ...
        if isinstance(self._tmp_norm_err, float):
            ...
            if self._tmp_norm/self._tmp_norm_err < self._sn_threshold:
                print("\n")
                print(f"S/N of {self._tmp_norm/self._tmp_norm_err} below threshold of {self._sn_threshold}")
                return -1
    ...
    def _paste_modelbox(self, data : np.ndarray, x : int, y : int, w : int, h : int,angle : float, offset : float):
        '''
    ...
    def build(self, stepsize : int = 1, steps : tuple = (9999,9999),liveplot : bool = False, verbose : int = 1):
        '''
        ...
        if len(steps) != 2:
            raise ValueError(f"false number of values to unpack (expected 2 got {len(steps)})")
        
        if verbose==1:
            util.intro()

        if liveplot:
            live_plot = LivePlot(real_data=self.masked_original_data, box_width=self.init_w,box_height=self.init_h)
        
        self._norm_tracker = ParamTracker()

        # fit the init box
        init_box = Box(self.masked_original_data, x = self.init_x, y = self.init_y, width = self.init_w, height = self.init_h, 
                       seeing=self._seeing, init=[self.init_angle,1,1,0,0,0,0,0,0], 
                       h2 = self._h2, skew=self._skew, h4=self._h4, fix_bg = self._fix_bg, nowst=self.nowst)
        ...
        # paste init box
        self._paste_modelbox(init_box.model, self.init_x, self.init_y, self.init_w, self.init_h,angle, init_box.offset)
        ...
        for run in range(2):
            ...
            for _ in range(steps[run]):
                ...
                x,y = util.correct_box_center_from_peak(x=x,y=y,w=w,h=h, peak_pos=self._tmp_peak_pos)
                
                x,y = util.calculate_next_boxcenter(angle = angle, x_center = x, y_center = y, direction = direction,
                                                    stepsize=stepsize, dictonary=slope_dict)
                
                if self._vary_box_dim:
                    w,h = util.calculate_new_box_dimensions(angle=angle,init_width=self.init_w,init_height=self.init_h,sigma = self._tmp_sigma)
                
                tmp_box = Box(self.masked_original_data, x , y , w, h, init = params, seeing=self._seeing, h2=self._h2, skew=self._skew, h4=self._h4, fix_bg = self._fix_bg, nowst=self.nowst)
    ...
    def show(self, output: str = None):
        ...
        fig, (ax1, ax2, ax3) = plt.subplots(1,3, sharex=True,sharey=True,figsize=(12,4),subplot_kw={'projection':wcs})
        ...
        # residual
        ax3.imshow(self.masked_original_data-model, vmin=vmin, vmax=vmax, cmap="YlOrBr", origin='lower')
        ax3.set_xlabel("R.A.")
        ax3.set_ylabel("Dec.")
    ...
    def _save(self):
        ...
        hdul = fits.HDUList([primary_hdu, mask_hdu, mrdata_hdu, residual_hdu,model_hdu])
    ...
    def _create_parameter_table(self):
        ...
        oec = fits.Column(name=f"offset_{self._filter}_err", format="E", array=offset_err)
        ...
        skewec = fits.Column(name=f"skew_{self._filter}_err", format="E", array=skew_err)
        ...
        hdu = fits.BinTableHDU.from_columns([bxc,byc,bwc,bhc,ac,aec,sc,sec,nc,nec,oc,oec,xc,xec,yc,yec,h2c,h2ec,skewc,skewec,h4c,h4ec])
