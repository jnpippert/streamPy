...
from .utilities import create_grid_arrays, calc_fwhm_pos
# other imports
...
class Box:
    ...
    def __init__(self, original_data : np.ndarray,
        ...
        func_params = func_model.make_params(angle=iangle,sigma=isigma,norm=inorm,offset=ioffset,
                                             x0=ix0,y0=iy0,h2v=ih2,skewv=iskew,h4v=ih4)
    ...
    def _fitfunc(self, xy : np.ndarray, angle : float, sigma : float, norm : float, offset : float, 
                 x0 : float, y0 : float, h2v : float, skewv : float, h4v : float) -> np.ndarray:
        ...
        '''
        grid = np.sin(np.radians(angle)) * (self._y_grid-y0) + np.cos(np.radians(angle)) * (self._x_grid-x0)
        ...
        model = (norm * np.exp(-0.5 * vals**2) * (1 + h2_comp + h4_comp + scn.cdf(skewv*vals))) + offset
        if self._nan_policy:
            return convolve(model, kernel=self._psf, boundary="extend",nan_treatment="fill")
        return convolve(model, kernel=self._psf, boundary="extend",nan_treatment="fill")
    ...
    def fit_model(self):
        ...
        try:
            result = self._func_model.fit(ravel_data, self._func_params, xy=np.arange(ravel_data.size)) # xy is just a place holder
        except:
            return -1
        ...
        self.angle, self.sigma, self.norm, self.offset, self.x0, self.y0, self.h2v, self.skewv, self.h4v = self.params
...
class BoxList:
    ...
    def __init__(self, filename : str = None):
        ...
        file.write("#id xpos ypos width height angle sigma norm offset x0 y0 h2 h3 h4\n")
        file.close()
