...
class Slice(Stretch):
    ...
    def __init__(self, filename : str, paramfile : str):
        ...
        self._img = self._iax.imshow(self._stretched_data, vmin = self._vmin, vmax = self._vmax, origin = "lower", cmap="YlOrBr")
        ...
        self._simgd = self._isaxd.imshow(self._data_slice_arr, vmin = self._vmin, vmax = self._vmax, origin = "lower", cmap="YlOrBr")
        
        # model slice axis
        self._isaxm = self._fig.add_subplot(self._grid[1,3:],aspect=1)
        self._simgm = self._isaxm.imshow(self._model_slice_arr, vmin = self._vmin, vmax = self._vmax, origin = "lower", cmap="YlOrBr")
        
        # plot axis
        self._pax = self._fig.add_subplot(self._grid[2,3:])
        self._plot_gauss, = self._pax.plot(self._gauss1d + self._offset,label="model")
        self._pax.set_ylim(np.nanmin(self._data1d-self._gauss1d)-1,np.nanmax(self._data1d) + 1)
        self._plot_data, = self._pax.plot(self._data1d, ".",label="data")
        self._plot_res, = self._pax.plot(self._data1d-self._gauss1d, color = "k", lw=0.8,label="residual")
        self._plot_meanres = self._pax.axhline(np.nanmean(self._data1d-self._gauss1d), color = "k", lw=0.8, linestyle="dashed")
        self._plot_aper,  = self._pax.plot(self._aper1d,color="k",ls="dotted",label="mask")
        self._plot_hm = self._pax.axhline(np.max(self._gauss1d)/2,lw = 0.8)
        # point
        self._point, = self._iax.plot([self._x],[self._y], ".", color ="red")

        # rectangle
        self._rect = self._iax.add_patch(Rectangle([self._x-self._width,self._y-self._height],width = 2*self._width+1,height=2*self._height+1,fill=False,color = "red", linewidth = 0.8))
        
        # initialize width slider
        ax_width = self._fig.add_axes([0.03,0.2,0.01,0.7])
        self._width_slider = Slider(ax = ax_width, label = 'width', 
                             valmin = 0, 
        ...
        self._height_slider = Slider(ax = ax_height, label = 'height', 
                             valmin = 0, 
        ...
        self._cid = self._point.figure.canvas.mpl_connect('button_press_event', self._mouse_click)
    ...
    def _update_slices(self):
        ...
        self._pax.set_ylim(np.nanmin(self._data1d-self._gauss1d)-mv/5,np.nanmax(self._data1d) + mv/5)
    ...
    def _slice(self):
        self._model_slice_arr = self._stretched_model[self._y-self._height:self._y+self._height,self._x-self._width:self._x+self._width]
        self._data_slice_arr = self._stretched_data[self._y-self._height:self._y+self._height,self._x-self._width:self._x+self._width]
        ...
        if self._width > self._height:
            self._gauss1d = self._model[self._y,self._x-self._width:self._x+self._width]
            self._aper1d = self._aperture[self._y,self._x-self._width:self._x+self._width]
        else:
            self._gauss1d = self._model[self._y-self._height:self._y+self._height,self._x]
            self._aper1d = self._aperture[self._y-self._height:self._y+self._height,self._x]
        
        
        self._data1d = self._stack_data_slices(w=self._width,h=self._height,x=self._x,y=self._y)
    ...
    def _mean_box(self, array):
        ...
        yg,xg = np.ogrid[0 - self._height : self._height, 0 - self._width : self._width]
        grid = np.sin(np.radians(angle)) * (yg-y0) + np.cos(np.radians(angle)) * (xg-x0)
        xl = []
        yl = []
...
class Image(Stretch):
    ...
    def __init__(self, filename : str, cmap : str = "YlOrBr", color = "red"):
        super.__init__(filename)

        raise NotImplementedError
        # TODO implement vmin and vmax sliders
        # TODO implement scale buttons
        # TODO implement 